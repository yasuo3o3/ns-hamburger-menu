...
class NS_Hamburger_Menu {
	...
	public function add_settings_page() {
		add_options_page( 'NS Hamburger Menu', 'NS Hamburger Menu', 'manage_options', 'ns-hamburger-menu', array( $this, 'settings_page_html' ) );
	}
	public function admin_assets( $hook ) {
		...
		wp_enqueue_script( 'ns-hmb-admin', plugin_dir_url( __FILE__ ) . 'assets/admin.js', array( 'jquery', 'wp-color-picker' ), self::VER, true );
	}
	...
	public function front_assets() {
		...
		wp_enqueue_style( 'ns-hmb-style', plugin_dir_url( __FILE__ ) . 'assets/css/ns-hamburger.css', array(), self::VER );
		$inline = sprintf(
			...
			esc_html( $c_end ),
			intval( $opt['columns'] ),
			intval( $opt['top_font_px'] ),
			intval( $opt['sub_font_px'] ),
			intval( $opt['hue_speed_sec'] ),
			intval( $opt['hue_range_deg'] ) . 'deg',
			intval( $opt['open_speed_ms'] ),
			intval( $opt['z_index'] ),
			esc_html( $opt['responsive_mode'] ),
			intval( $opt['responsive_width'] )
		);
		wp_add_inline_style( 'ns-hmb-style', $inline );

		wp_enqueue_script( 'ns-hmb-script', plugin_dir_url( __FILE__ ) . 'assets/js/ns-hamburger.js', array(), self::VER, false );
		if ( function_exists( 'wp_script_add_data' ) ) {
			wp_script_add_data( 'ns-hmb-script', 'strategy', 'defer' );
		}
		wp_localize_script( 'ns-hmb-script', 'NS_HMB', array( 'hueAnimDefault' => (int) $opt['hue_anim'] ) );
	}

	private function get_scheme_colors( $scheme ) {
		switch ( $scheme ) {
			case 'blue':
				return array( '#0ea5e9', '#60a5fa' );
			case 'green':
				return array( '#22c55e', '#86efac' );
			case 'red':
				return array( '#ef4444', '#f87171' );
			case 'orange':
				return array( '#f59e0b', '#fdba74' );
			case 'black':
				return array( '#0b0b0b', '#575757' );
			default:
				return null;
		}
	}
	...
	public function settings_page_html() {
		...
		<table class="form-table" role="presentation">
			...
			<select id="ns_scheme" name="<?php echo esc_attr( $name . '[scheme]' ); ?>">
			<?php
				$schemes = array(
			...
			<tr><th>列数 / フォント</th><td>
			列数：<select name="<?php echo esc_attr( $name . '[columns]' ); ?>">
			<?php
			foreach ( range( 1, 6 ) as $c ) {
				printf( '<option value="%1$s"%3$s>%2$s 列</option>', esc_attr( (int) $c ), esc_html( (int) $c ), selected( $opt['columns'], (int) $c, false ) ); }
			?>
			</select>
			<div style="margin-top:8px">
	...
	public function register_block() {
		register_block_type( __DIR__ . '/blocks', array( 'render_callback' => array( $this, 'render_block' ) ) );
	}

	// ← ここがポイント：$block が無い環境でも $content からスロットを拾う
	public function render_block( $attributes = array(), $content = '', $block = null ) {
		$attrs = array();
		foreach ( array( 'columns', 'topFontPx', 'subFontPx', 'colorStart', 'colorEnd', 'hueAnim', 'hueSpeedSec', 'zIndex' ) as $k ) {
	...
	private function split_slots_from_block( $block ) {
		...
		foreach ( $block->inner_blocks as $child ) {
			...
			foreach ( $child->inner_blocks as $ib ) {
				if ( ! empty( $ib->parsed_block ) ) {
					$html .= render_block( $ib->parsed_block );
				} elseif ( method_exists( $ib, 'render' ) ) {
					$html .= $ib->render();
				}
			}
			if ( $pos === 'after' ) {
	...
	private function split_slots_from_content( $content ) {
		...
		foreach ( $blocks as $b ) {
			if ( ( $b['blockName'] ?? '' ) !== 'ns/hamburger-slot' ) {
				continue;
			}
	...
	private function render_markup( $return_string = true, $attrs = array(), $block = null, $content = '' ) {
		...
		$open_shape = isset( $attrs['openShape'] ) && in_array( $attrs['openShape'], array( 'circle', 'linear' ), true ) ? $attrs['openShape'] : ( $opt['open_shape'] ?? 'circle' );
		$style_vars = sprintf(
			'--ns-start:%1$s;--ns-end:%2$s;--ns-columns:%3$d;--ns-top-fz:%4$spx;--ns-sub-fz:%5$spx;--ns-hue-speed:%6$ss;--ns-open-speed:%7$sms;--ns-z:%8$d;--ns-resp-mode:%9$s;--ns-resp-width:%10$dpx;',
			esc_attr( $c_start ),
			esc_attr( $c_end ),
			$columns,
			$top_fz,
		...
		<div id="<?php echo esc_attr( $overlay_id ); ?>" class="ns-overlay<?php echo $hue_on ? '' : ' ns-hue-off'; ?>" hidden style="<?php echo esc_attr( $style_vars ); ?>">
			<div class="ns-overlay__inner">
				<nav class="ns-overlay__nav" aria-label="<?php esc_attr_e( 'Hamburger menu', 'ns-hamburger-menu' ); ?>">
					...
					if ( $menu ) {
						echo wp_kses_post( $menu ); } elseif ( current_user_can( 'edit_theme_options' ) ) {
...
// TEMPORARILY DISABLED: Testing new architecture only
